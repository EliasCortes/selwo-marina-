...
App.Views = (() => {
  ...
  const FISH_TYPES = [
    { key: 'arenque_peq', label: 'Arenque pequeño', color: '#00b4d8', emoji: '�x�x' },
    { key: 'arenque_gde', label: 'Arenque grande', color: '#0077b6', emoji: '�x�x' },
    { key: 'sprat', label: 'Sprat', color: '#90e0ef', emoji: '�x��' },
    { key: 'capelin', label: 'Capelin', color: '#f59e0b', emoji: '�x��' },
    { key: 'caballa', label: 'Caballa', color: '#ef4444', emoji: '�x��' },
    { key: 'bacaladilla', label: 'Bacaladilla', color: '#8b5cf6', emoji: '�x�x' },
  ];
  ...
  async function renderMenu() {
    ...
    const { getAnimalsCount } = await import('../src/services/animalService.js?v=9');
  ...
  async function renderAnimalList(params) {
    ...
    // Cargar dinámicamente el servicio de animales de Supabase
    const { getAnimals } = await import('../src/services/animalService.js?v=9');
  ...
  async function renderDeptSection(params) {
    ...
    if (sectionId === 'diets') {
      const { getAllDietRecords, getAnimals } = await import('../src/services/animalService.js?v=9');
      try {
    ...
    app.innerHTML = `
      ...
      return `<tr>
        ...
        <div style="margin-top:var(--sp-6);text-align:center;">
          <button class="btn btn-outline" onclick="App.Views.exportDeptData('${deptId}')">�x� Exportar Datos (JSON)</button>
        </div>
      </main>
    `;
  ...
  // ���� Global Training View (SaaS Style) ����������������������������������������������
  async function renderGlobalTrainingView(params) {
    ...
    app.innerHTML = `
      ...
      <main class="main-content">
        <div class="modern-training-header">
          ...
          <button class="btn btn-primary" onclick="App.Views.openDeptRecordForm('trainings', '${deptId}')">+ Nuevo Registro</button>
        </div>
        
        <div class="modern-training-container">
          ${summaryHtml}
          
          <div class="training-chart-card" style="margin-bottom: var(--sp-4);">
            <h4>�x� Evolución de Sesiones</h4>
            <div class="training-chart-container" style="height: 220px;">
              <canvas id="global-training-chart"></canvas>
            </div>
          </div>
          
          <div class="card">
            <div class="card-header">
              <h3>�altimas Sesiones</h3>
            </div>
            <div class="card-body" style="padding: 0 var(--sp-4);">
    ...
    if (records.length > 0) {
      // Reusamos la lógica pero le pasamos el ID del canvas global
      renderTrainingBarChart(records, 'global-training-chart', 'line');
    }
  }
  ...
  async function renderAnimalCard(params) {
    ...
    // Cargar dinámicamente el servicio de animales de Supabase
    const { getAnimalById } = await import('../src/services/animalService.js?v=9');
  ...
  async function renderTabContent(animal, tab) {
    ...
    switch (tab) {
      ...
      case 'diets':
        ...
        break;
      case 'trainings':
        await renderTrainingTab(container, animal);
        break;
  ...
  function renderGeneralTab(animal) {
    return `
      <div class="card">
        <div class="card-header">
          <h3>�x9 Datos Generales</h3>
          <div class="btn-group">
            <button class="btn btn-outline btn-sm" onclick="App.Views.openAnimalForm('${animal.department}', '${animal.id}')">�S�️ Editar</button>
            <button class="btn btn-danger btn-sm" onclick="App.Views.deleteAnimal('${animal.id}')">�x️ Eliminar Animal</button>
          </div>
        </div>
        <div class="card-body">
          <div class="info-grid">
  ...
  // �"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"�
  //  LEONES MARINOS � Módulo de Entrenamientos Estilo Excel
  // �"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"��"�

  const ATTITUDE_OPTIONS = [
    { value: 'Excelente', emoji: '�xx�', label: 'Excelente', css: 'excelente', color: '#4CAF50' },
    { value: 'Regular',   emoji: '�xx�', label: 'Regular',   css: 'regular',   color: '#FFC107' },
    { value: 'Mal',       emoji: '�xx�', label: 'Mal',       css: 'mal',       color: '#FF9800' },
    { value: 'Muy Mal',   emoji: '�xx�', label: 'Muy Mal',   css: 'muy-mal',   color: '#F44336' },
  ];

  let trainingBarChart = null;
  let trainingPieChart = null;

  function getAttitudeBadge(value) {
    const opt = ATTITUDE_OPTIONS.find(o => o.value === value);
    if (!opt) return `<span class="attitude-badge">${H.escapeHtml(value || '�')}</span>`;
    return `<span class="attitude-badge attitude-badge--${opt.css}">${opt.emoji} ${opt.label}<span class="attitude-dropdown-indicator">��</span></span>`;
  }

  async function renderTrainingTab(container, animal) {
    const {
      getTrainingRecords, getAnimals
    } = await import('../src/services/animalService.js?v=9');

    // Obtener registros de entrenamiento de este animal
    const records = await getTrainingRecords(animal.id, 100);

    // Obtener lista de animales del departamento para las pestañas
    let deptAnimals = [];
    try {
      const { data } = await getAnimals({ departamentoId: animal.department });
      deptAnimals = (data || []).map(a => ({ id: a.id, name: a.nombre }));
    } catch (err) {
      console.error('Error al obtener animales del departamento:', err);
    }

    // ���� Estadísticas de Actitud ������������������������������������������������������������
    const attitudeCounts = { 'Excelente': 0, 'Regular': 0, 'Mal': 0, 'Muy Mal': 0 };
    records.forEach(r => {
      const val = r.resultado || '';
      if (attitudeCounts[val] !== undefined) attitudeCounts[val]++;
    ...
    const actionsHtml = `
      <div class="training-actions">
        <button class="btn btn-primary" onclick="App.Views.openTrainingForm('${animal.id}', '${animal.department}')">
          + Registrar Entrenamiento
        </button>
      </div>
    `;

    // ���� Listado Moderno de Tarjetas ����������������������������������������������������
    const dataCardsHtml = records.length > 0
      ? records.map(r => `
          <div class="modern-session-card">
            <div class="modern-session-card-header">
              <div class="msc-meta">
                <span class="msc-date" style="font-weight:600; color:var(--primary-color);">�x& ${H.formatDateTime(r.fecha)} � Sesión ${r.numero_sesion || 1}</span>
                <span class="msc-trainer">�x� ${H.escapeHtml(r.conducta_entrenada || 'Sin entrenador asignado')}</span>
              </div>
              <div class="msc-actions">
                <button class="btn btn-ghost btn-icon btn-sm" onclick="App.Views.openTrainingForm('${animal.id}', '${animal.department}', '${r.id}')" title="Editar">�S�️</button>
                <button class="btn btn-ghost btn-icon btn-sm text-danger" onclick="App.Views.deleteRecord('trainings', '${r.id}')" title="Eliminar">�x️</button>
              </div>
            </div>
            ${r.comentarios ? `
              <div class="modern-session-card-body">
                ${H.escapeHtml(r.comentarios)}
              </div>
            ` : ''}
            <div class="modern-session-card-footer">
              ${getAttitudeBadge(r.resultado)}
            </div>
          </div>
        `).join('')
      : '<p class="text-muted" style="padding: 2rem 0; text-align: center;">No hay entrenamientos registrados para este animal.</p>';

    const listHtml = `
      <div class="modern-session-card-list">
        ${dataCardsHtml}
      </div>
    `;

    // ���� Pestañas inferiores tipo SaaS ������������������������������������������������
    const tabsHtml = `
      <div class="training-tabs-bar">
        ${deptAnimals.map(a => `
          <div class="training-tab training-tab--animal ${a.id === animal.id ? 'training-tab--active' : ''}"
               onclick="App.Router.navigate('/animal/${a.id}/trainings')">
            �x�� ${H.escapeHtml(a.name)}
          </div>
        `).join('')}
        <div class="training-tab training-tab--training training-tab--active">
          �x9 Entrenamiento
        </div>
      </div>
    `;

    // ���� Gráfico Individual (Donut) ������������������������������������������������������
    const chartsHtml = totalRecords > 0 ? `
      <div class="training-charts-row">
        <div class="training-chart-card">
          <h4>�x` Distribución de Actitud</h4>
          <div class="training-chart-container" style="height: 250px;">
    ...
    // ���� Renderizar todo ����������������������������������������������������������������������������
    container.innerHTML = actionsHtml + chartsHtml + listHtml + tabsHtml;

    // ���� Inicializar gráficos ������������������������������������������������������������������
    if (totalRecords > 0) {
      renderTrainingPieChart(attitudeCounts, totalRecords);
    }
  }

  // ���� Gráfico de Barras/Líneas: Sesiones por Mes ����������������������������������������
  function renderTrainingBarChart(records, canvasId = 'training-bar-chart', type = 'bar') {
    const ctx = document.getElementById(canvasId);
    if (!ctx) return;

    if (trainingBarChart) { trainingBarChart.destroy(); trainingBarChart = null; }

    // Agrupar sesiones por mes (últimos 7 meses)
    const monthCounts = {};
    const now = new Date();
    for (let i = 6; i >= 0; i--) {
    ...
    const labels = Object.keys(monthCounts).map(k => {
      const [, m] = k.split('-');
      return monthNames[parseInt(m) - 1];
    });
    const data = Object.values(monthCounts);

    trainingBarChart = new Chart(ctx, {
      type: type,
      data: {
        labels,
        datasets: [{
          label: 'Sesiones',
          data,
          backgroundColor: type === 'line' ? 'rgba(0, 119, 182, 0.1)' : 'rgba(0, 119, 182, 0.7)',
          borderColor: '#0077b6',
  ...
  // ���� Gráfico de Pastel: Distribución de Actitud ��������������������������
  function renderTrainingPieChart(attitudeCounts, total) {
    const ctx = document.getElementById('training-pie-chart');
    if (!ctx) return;

    if (trainingPieChart) { trainingPieChart.destroy(); trainingPieChart = null; }

    const labels = ATTITUDE_OPTIONS.map(o => o.label);
    const data = ATTITUDE_OPTIONS.map(o => attitudeCounts[o.value] || 0);
    const colors = ATTITUDE_OPTIONS.map(o => o.color);

    trainingPieChart = new Chart(ctx, {
      type: 'doughnut',
      data: {
        labels,
        datasets: [{
            ...
            labels: {
              ...
              generateLabels: (chart) => {
                ...
                return chart.data.labels.map((label, i) => {
                  ...
                  return {
                    text: `${ATTITUDE_OPTIONS[i].emoji} ${label}: ${value} (${pct}%)`,
  ...
  // ���� Formulario de Entrenamiento (Modal) ����������������������������������������
    ...
    const summaryHtml = latest ? `
      <div class="card leo-diet-summary">
        <div class="leo-diet-summary-header">
          <h3>�x�x Dieta Actual</h3>
          <span class="leo-diet-summary-date">�altima actualización: ${H.formatDate(latest.fecha)}</span>
        </div>
    ...
    ` : `
      <div class="card leo-diet-summary leo-diet-summary--empty">
        <div class="empty-state" style="padding:var(--sp-8);">
          <div class="empty-state-icon">�x�x</div>
        ...
        return `
                      <tr class="${isToday ? 'leo-dt-today' : ''}">
                        ...
                        <td class="actions-cell">
                          <button class="btn btn-ghost btn-icon" onclick="App.Views.openLeonesDietForm('${animal.id}', '${animal.department}', '${r.id}')" title="Editar">�S�️</button>
                          <button class="btn btn-ghost btn-icon" onclick="App.Views.deleteRecord('diets', '${r.id}')" title="Eliminar">�x️</button>
                        </td>
                      </tr>
  ...
  async function renderLeonesDietChart(animalId, rangeDays) {
    ...
    const { getDietRecordsByDateRange, getDietRecords } = await import('../src/services/animalService.js?v=9');
  ...
  async function openLeonesDietForm(animalId, deptId, recordId = null, copyPrevious = false) {
    const { getLatestDietRecord, createDietRecord, updateDietRecord, getRecordById } = await import('../src/services/animalService.js?v=9');
    ...
    UI.showModal({
      title: isEdit ? '�S�️ Editar Dieta' : (copyPrevious ? '�x9 Copiar Dieta Anterior' : '+ Registrar Dieta del Día'),
      contentHtml: formHtml,
    ...
    tbody.innerHTML = (Array.isArray(App.Views.currentDietSessions) ? App.Views.currentDietSessions : []).map((session, index) => {
      ...
      return `
        <tr>
          ...
          <td>
            <button type="button" class="btn btn-ghost btn-icon" style="color:var(--danger-500); width:28px; height:28px;" onclick="App.Views.removeLeoDietSession(${index})" title="Eliminar sesión">�x️</button>
          </td>
        </tr>
      `;
    }).join('');
  ...
  function renderLeoDietExtras() {
    ...
    container.innerHTML = App.Views.currentDietExtras.map((extra, index) => `
      <div style="display: grid; grid-template-columns: 1fr 1fr auto; gap: var(--sp-3); align-items: end; margin-bottom: var(--sp-3);">
        ...
        <button type="button" class="btn btn-ghost btn-icon" onclick="App.Views.removeLeoDietExtra(${index})" title="Eliminar" style="color: var(--danger-500);">�x️</button>
      </div>
    `).join('');
  }
  ...
  async function renderDietTab(container, animal) {
    ...
    const { getRecordsByAnimal } = await import('../src/services/animalService.js?v=9');
    let records = [];
    try {
    ...
    container.innerHTML = `
      <div class="card diet-breakdown-card">
        <div class="card-header">
          <h3>�x�x Desglose de Dieta por Pescado</h3>
        </div>
        <div class="card-body">
      ...
      return `
          ...
          <div style="text-align: right; margin-top: var(--sp-4);">
            <button class="btn btn-primary" onclick="App.Views.saveDailyDiet('${animal.id}')">�x� Guardar Registro Diario</button>
          </div>
        </div>
      </div>
  ...
  async function saveDailyDiet(animalId) {
    ...
    // Cargar dinámicamente el servicio de animales de Supabase
    const { createDietRecord } = await import('../src/services/animalService.js?v=9');
            ...
            <div class="attachments-list">
              ${allAttachments.map(att => `
                <div class="attachment-item">
                  <span class="attachment-icon">${att.mime_type && att.mime_type.includes('pdf') ? '�x' : '�x�️'}</span>
                  ...
                  <div class="attachment-actions">
                    ...
                    <button class="btn btn-ghost btn-icon" onclick="App.Views.deleteAttachment('${att.id}', '${animal.id}')" title="Eliminar">�x️</button>
                  </div>
                </div>
              `).join('')}
            </div>
    ...
    container.innerHTML = `
      <div class="card">
        <div class="card-header">
          <h3>${section.icon} ${section.name}</h3>
          <div class="btn-group">
            <button class="btn btn-outline btn-sm" onclick="App.Views.openAttachmentUploader('${animal.id}')">�x} Adjuntar</button>
    ...
    const html = `
      <div class="attachment-uploader">
        <label class="attachment-drop-zone" for="attachment-file-input" id="attachment-drop-zone">
          <div class="attachment-drop-icon">�x}</div>
        ...
        if (file && preview) {
          preview.innerHTML = `
            <div class="attachment-file-preview">
              <span>${file.type.includes('pdf') ? '�x' : '�x�️'}</span>
    ...
    container.innerHTML = `
      ...
      <div class="weight-stats-grid">
        <div class="weight-stat-card weight-stat--last">
          <div class="weight-stat-icon">�x`</div>
          <div class="weight-stat-value">${H.formatWeight(lastW, unit)}</div>
          <div class="weight-stat-label">�altimo Peso</div>
        </div>
        ...
        <div class="weight-stat-card weight-stat--min">
          <div class="weight-stat-icon">�x0</div>
        ...
        <div class="weight-stat-card weight-stat--avg">
          <div class="weight-stat-icon">�a️</div>
  ...
  async function openRecordForm(type, animalId, recordId = null) {
    ...
    const { getRecordById, updateSupabaseRecord, createSupabaseRecord } = await import('../src/services/animalService.js?v=9');
  ...
  async function deleteRecord(type, recordId) {
    ...
    const { deleteSupabaseRecord } = await import('../src/services/animalService.js?v=9');
    UI.showConfirm(
  ...
  async function openAnimalForm(deptId, animalId = null) {
    ...
    const { getAnimalById, createAnimal, updateAnimal, uploadAnimalPhoto } = await import('../src/services/animalService.js?v=9');
  ...
  async function deleteAnimal(animalId) {
    const { getAnimalById, deleteAnimal: deleteSupabaseAnimal } = await import('../src/services/animalService.js?v=9');
  ...
  async function openDeptRecordForm(type, deptId) {
    if (type === 'trainings') {
      return App.Views.openTrainingForm(null, deptId);
    }

    const { getAnimals, createSupabaseRecord } = await import('../src/services/animalService.js?v=9');
